...
namespace frutas.Services
{
    ...
    /// Define las operaciones de negocio para la gestión de logs y auditoría
    /// </summary>
    public interface ILogService
    {
        // Consultas
        ResponseDto<LogDto> ObtenerPorId(int id);
        ResponseDto<IEnumerable<LogDto>> ObtenerTodos(int pagina = 1, int tamañoPagina = 20);
        ...
        ResponseDto<IEnumerable<LogDto>> ObtenerPorUsuario(int usuarioId, int pagina = 1, int tamañoPagina = 20);
        ResponseDto<IEnumerable<LogDto>> ObtenerPorFecha(DateTime fecha);
        ResponseDto<IEnumerable<LogDto>> ObtenerErrores(int pagina = 1, int tamañoPagina = 20);
        ...
        // Logging público para otros servicios
    ...
    /// Contiene toda la lógica de negocio relacionada con auditoría y logging
    /// </summary>
    public class LogService : ILogService
    {
        ...
        public ResponseDto<IEnumerable<LogDto>> ObtenerTodos(int pagina = 1, int tamañoPagina = 20)
        {
            try
            {
                ...
                var logs = _logRepository.ObtenerPaginado(pagina, tamañoPagina);
        ...
        public ResponseDto<IEnumerable<LogDto>> ObtenerConFiltros(LogFiltroDto filtro)
        {
            try
            {
                ...
                if (filtro.TamañoPagina < 1 || filtro.TamañoPagina > 100) filtro.TamañoPagina = 20;
        ...
        public ResponseDto<IEnumerable<LogDto>> ObtenerPorUsuario(int usuarioId, int pagina = 1, int tamañoPagina = 20)
        {
            try
            {
                ...
                var logs = _logRepository.ObtenerPorUsuario(usuarioId).Skip((pagina - 1) * tamañoPagina).Take(tamañoPagina);
                var logsDto = logs.Select(MapearALogDto);
        ...
        public ResponseDto<IEnumerable<LogDto>> ObtenerErrores(int pagina = 1, int tamañoPagina = 20)
        {
            try
            {
                if (!SessionHelper.EsAdministrador)
                ...
                var logs = _logRepository.ObtenerErrores().Skip((pagina - 1) * tamañoPagina).Take(tamañoPagina);
                var logsDto = logs.Select(MapearALogDto);
        ...
        public ResponseDto<Dictionary<string, int>> ObtenerEstadisticasPorSeveridad()
        {
            try
            {
                if (!SessionHelper.EsAdministrador)
                {
                    return ResponseDto<Dictionary<string, int>>.Error("No tiene permisos para ver estadísticas");
                }
            ...
            {
                return ResponseDto<Dictionary<string, int>>.Error($"Error obteniendo estadísticas por severidad: {ex.Message}");
            }
        }
        ...
        public ResponseDto<Dictionary<string, int>> ObtenerEstadisticasPorAccion()
        {
            try
            {
                ...
                // Implementación simplificada - en un sistema real usaríamos GROUP BY
                var estadisticas = new Dictionary<string, int>();
            ...
            {
                return ResponseDto<Dictionary<string, int>>.Error($"Error obteniendo estadísticas por acción: {ex.Message}");
            }
        }
        ...
        public ResponseDto<Dictionary<string, int>> ObtenerEstadisticasPorTabla()
        {
            ...
            {
                return ResponseDto<Dictionary<string, int>>.Error($"Error obteniendo estadísticas por tabla: {ex.Message}");
            }
        }
        ...
        public ResponseDto<int> ContarLogsPorUsuario(int usuarioId)
        {
            try
            {
                ...
                if (!SessionHelper.EsAdministrador && SessionHelper.UsuarioActualId != usuarioId)
                {
                    return ResponseDto<int>.Error("No tiene permisos para ver estadísticas de este usuario");
                }
        ...
        public ResponseDto<int> ContarErroresHoy()
        {
            try
            {
                if (!SessionHelper.EsAdministrador)
                {
                    return ResponseDto<int>.Error("No tiene permisos para ver estadísticas de errores");
                }
        ...
        public ResponseDto LimpiarLogsAntiguos(int diasAMantener = 90)
        {
            try
            {
                ...
                RegistrarInfo("LOGS_CLEANED", "Logs", 
                    $"Logs anteriores a {diasAMantener} días fueron limpiados", 
                    SessionHelper.UsuarioActualId);

                return ResponseDto.Exito($"Logs anteriores a {diasAMantener} días han sido limpiados");
            }
            catch (Exception ex)
            {
            ...
            string detalleDepues = null, string mensajeError = null, bool exitoso = true)
        {
            ...
            catch
            {
                ...
                // En un sistema de producción, podrías escribir a un archivo de log de emergencia
            }
        }
