...
namespace frutas.Services
{
    ...
    public class UsuarioService : IUsuarioService
    {
        ...
        private const int SEGUNDOS_BLOQUEO = 30; // Cambiado de 15 minutos a 30 segundos para pruebas
            ...
            try
            {
                ...
                if (usuario.CuentaBloqueada)
                {
                    if (usuario.FechaBloqueo.HasValue && 
                        DateTime.Now.Subtract(usuario.FechaBloqueo.Value).TotalSeconds < SEGUNDOS_BLOQUEO)
                    {
                        var tiempoRestante = SEGUNDOS_BLOQUEO - (int)DateTime.Now.Subtract(usuario.FechaBloqueo.Value).TotalSeconds;
                        RegistrarLog("LOGIN_BLOCKED", "Usuarios", $"Intento de login con cuenta bloqueada: {loginDto.Username}");
                        return ResponseDto<AuthResponseDto>.Error($"Cuenta bloqueada. Intente nuevamente en {tiempoRestante} segundos");
                    }
                    else
                    {
                ...
                if (!PasswordHelper.VerificarPassword(loginDto.Password, usuario.PasswordHash, usuario.Salt))
                {
                    ...
                    if (usuario.IntentosFallidos >= MAX_INTENTOS_LOGIN)
                    {
                        ...
                        return ResponseDto<AuthResponseDto>.Error($"Cuenta bloqueada por {SEGUNDOS_BLOQUEO} segundos debido a múltiples intentos fallidos");
                    }

                    var intentosRestantes = MAX_INTENTOS_LOGIN - usuario.IntentosFallidos;
                    RegistrarLog("LOGIN_FAILED", "Usuarios", $"Contraseña incorrecta para: {loginDto.Username}. Intentos restantes: {intentosRestantes}");
                    return ResponseDto<AuthResponseDto>.Error($"Credenciales inválidas. Le quedan {intentosRestantes} intento(s) antes del bloqueo");
                }
